...
function Invoke-Import($payload) {
  ...
  foreach ($g in $groups) {
    foreach ($it in $g.items) {
      if (-not $it) { continue }
      $xml = [string]$it.xml
      if ($xml -notmatch '^\s*<(VOUCHER|LEDGER|GROUP)\b') { $results += [ordered]@{ id = $it.id; kind = $g.kind; ok = $false; message = 'Only VOUCHER, LEDGER or GROUP objects can be posted.' }; continue }
